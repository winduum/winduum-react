# [Details](https://winduum.dev/docs/components/details.html)

## Installation
By default, we recommend copying and pasting the code from this directory to your project and remapping the imports to local.

```js
import { Details } from '@/components/ui/details'
```
### NPM
Alternatively, you can also import directly from `npm` so you can leverage updates.
```shell
npm i winduum-react
```

```js
import { Details } from 'winduum-react/src/components/details'
```

### Docs
Visit [docs](https://winduum.dev/docs/components/details.html) to learn more and see usage examples.
